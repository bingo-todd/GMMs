...
class GMMs(object):
        ...
        for iter in range(1, self.max_iter+1):
            try:
                ...
                for model_i in range(k):
                    ...
                    sigma_all[model_i] = ((np.dot(
                                            np.multiply(diff_array.T,
                                                        posterior[:, model_i]),
                                            diff_array)
                                           / pd_data_model_i)
    ...
    def plot(self, ax, x, mu_all, sigma_all, pi_all):
        ...
        # levels = np.logspace(np.log(np.min(p)), np.log(np.max(p)), 100)
        levels = np.linspace(np.min(p), np.max(p), 100)
        index_round = np.argmin(np.abs(p[:, np.newaxis]-levels[np.newaxis, :]),
                                axis=1)
        for i in range(100):
            p[index_round == i] = levels[i]

        p = p.reshape([x0.shape[0], x1.shape[0]])
        plot_tools.plot_contour(ax, x0, x1, p,
